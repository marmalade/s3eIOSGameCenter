...
// orientation.
//
// Since Airplay uses a fixed portrait UIView, these GUIs would always be in
// portrait even if the iPad is held in landscape. A dummy view is
// positioned between the main airplay view and the modal views and allowed to
...
- (BOOL)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    s3eConfigGetInt("s3e", "FixScreenOrientation", &fixOrientation);
...
static void (^s3eInviteHandler)(GKInvite*, NSArray*) = ^(GKInvite *gkinvite, NSArray *playersToInvite)
{
    ...
    s3eConfigGetInt("s3e", "iPhoneGameCenterInviteCancelsMatchmaking", &cancelMatchmaking);
